...
export function parseIngredientString(ingredientString) {
...
export function assignCategory(ingredientName) {
...
// =================================================================================
export function generateShoppingList(plannedMeals, allRecipes, pantryItems = []) {
    ...
    plannedMeals.forEach(meal => {
        ...
        recipe.ingredients.forEach(ingredientString => {
            ...
            if (parsed.name === 'non-item' || parsed.quantity <= 0)
                return;
...
export const calculateTrendWeight = (dailyWeightLog) => {
    ...
    const trend_weight_data = sorted_logs.map((log, index, arr) => {
