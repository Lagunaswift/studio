...
export default function DietAndAllergensPage() {
    ...
    return (<PageWrapper title="Diet & Allergen Settings">
            ...
            <CardContent className="space-y-8">
               ...
               <div>
                  <h3 className="text-lg font-medium mb-3 text-primary">Dietary Preferences</h3>
                   <FormField control={form.control} name="dietaryPreferences" render={() => (<FormItem>
                        <div className="grid grid-cols-2 gap-4">
                          {AVAILABLE_DIETARY_PREFERENCES.map((preference) => (<FormField key={preference} control={form.control} name="dietaryPreferences" render={({ field }) => (<FormItem className="flex flex-row items-center space-x-3 space-y-0">
                                  <FormControl>
                                    <Checkbox checked={field.value?.includes(preference)} onCheckedChange={(checked) => {
               ...
               <div>
                   ...
                   <FormField control={form.control} name="allergens" render={() => (<FormItem>
                        <div className="grid grid-cols-2 gap-4">
                          {AVAILABLE_ALLERGENS.map((allergen) => (<FormField key={allergen} control={form.control} name="allergens" render={({ field }) => (<FormItem className="flex flex-row items-center space-x-3 space-y-0">
                                  <FormControl>
                                    <Checkbox checked={field.value?.includes(allergen)} onCheckedChange={(checked) => {
                        const newValue = checked
