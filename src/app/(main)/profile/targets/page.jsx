...
import { suggestProteinIntake } from '@/ai/flows/suggest-protein-intake-flow';
...
const getMonthlyGainKg = (sex, level) => {
    if (!sex || !level || level === 'notSpecified')
        return 0;
    if (sex === 'female') {
        switch (level) {
...
export default function DietaryTargetsPage() {
    ...
    const [proteinSuggestion, setProteinSuggestion] = useState(null);
    const [aiError, setAiError] = useState(null);
    const [fatSuggestion, setFatSuggestion] = useState(null);
    const [fatSuggestionError, setFatSuggestionError] = useState(null);
    const [weightLossWarning, setWeightLossWarning] = useState(null);
    const [energyAvailabilityWarning, setEnergyAvailabilityWarning] = useState(null);
    const [lossPercentage, setLossPercentage] = useState(0.75);
    ...
    useEffect(() => {
        const protein = parseFloat(proteinValue) || 0;
    ...
    useEffect(() => {
        ...
        if (tdee && weightKg && tdee > caloriesValue) {
            ...
            if (weeklyPercentageLoss > 1) {
                setWeightLossWarning(`A target of ${Math.round(caloriesValue)} kcal may lead to a weekly weight loss of over 1% of your body mass. Rapid weight loss can be unsustainable and may risk muscle loss. A daily deficit of around ${Math.round((weightKg * 0.01 * 7700) / 7)} kcal is generally a safer upper limit.`);
            }
            else {
                setWeightLossWarning(null);
            }
        ...
        if (sex === 'female' && leanBodyMassKg) {
            ...
            if (energyPerLbm < 30) {
                setEnergyAvailabilityWarning(`For females, an intake below 30-45 kcal per kg of lean body mass can increase health risks, including impacts on metabolic rate and menstrual function. Please consult a healthcare professional if you have concerns.`);
            }
            else {
                setEnergyAvailabilityWarning(null);
            }
    ...
    const handleApplyCalorieTarget = (calorieTarget) => {
    ...
    const handleMacroSubmit = (data) => {
        let newTargetRateKg = null;
    ...
    const handleGetProteinSuggestion = async (recommendationType) => {
    ...
    return (<PageWrapper title="Dietary Targets">
       <div className="grid md:grid-cols-2 gap-8">
        <div className="space-y-8">
            <Card>
                ...
                <form onSubmit={macroForm.handleSubmit(handleMacroSubmit)}>
                    ...
                    <FormField control={macroForm.control} name="protein" render={({ field }) => (<FormItem>
                    ...
                    {proteinSuggestion && !isAISuggesting && (<Card className="bg-secondary/50 p-4">
                    ...
                    <FormField control={macroForm.control} name="fat" render={({ field }) => (<FormItem>
                            <div className="flex justify-between items-center">
                            <FormLabel>Fat (g)</FormLabel>
                            <Button type="button" variant="outline" size="sm" onClick={handleGetFatSuggestion} disabled={!userProfile || !userProfile.tdee} title={!userProfile || !userProfile.tdee ? "Requires user info to calculate TDEE" : "Get Suggestion"}>
                    ...
                    {fatSuggestion && !fatSuggestionError && (<Card className="bg-secondary/50 p-4">
                        <CardHeader className="p-0 pb-2">
                            <CardTitle className="text-md flex items-center text-primary">
                    ...
                    <FormField control={macroForm.control} name="carbs" render={({ field }) => (<FormItem>
                    ...
                    <FormField control={macroForm.control} name="calories" render={({ field }) => (<FormItem>
                    ...
                    {(weightLossWarning || energyAvailabilityWarning) && (<div className="space-y-4 pt-4">
                        {weightLossWarning && (<Alert variant="destructive">
                            <AlertTriangle className="h-4 w-4"/>
                            <AlertTitle>Potential for Rapid Weight Loss</AlertTitle>
                            <AlertDescription>{weightLossWarning}</AlertDescription>
                            </Alert>)}
                        {energyAvailabilityWarning && (<Alert variant="destructive">
                            <AlertTriangle className="h-4 w-4"/>
            ...
            {aiError && (<Alert variant="destructive">
                <Info className="h-4 w-4"/>
                <AlertTitle>AI Suggestion Error</AlertTitle>
                <AlertDescription>{aiError}</AlertDescription>
                </Alert>)}
             {fatSuggestionError && (<Alert variant="destructive">
                  <Info className="h-4 w-4"/>
        ...
        <div className="space-y-8">
            <Card>
                ...
                <CardContent>
                    {!userProfile?.tdee ? (<Alert variant="destructive">
                            <Info className="h-4 w-4"/>
                        ...
                        </Alert>) : (<Tabs defaultValue={userProfile?.primaryGoal === 'muscleGain' ? 'gain' : 'loss'} className="w-full">
                            ...
                            <TabsContent value="loss" className="pt-4">
                                <div className="space-y-6">
                                    <div className="space-y-2">
                                        ...
                                        <Slider id="loss-slider" min={0.5} max={1.0} step={0.05} value={[lossPercentage]} onValueChange={(value) => setLossPercentage(value[0])} disabled={!fatLossCalculation.enabled}/>
                                    ...
                                    <Button type="button" onClick={() => handleApplyCalorieTarget(fatLossCalculation.calorieTarget)} className="w-full" disabled={!fatLossCalculation.enabled}>
                                        Apply This Calorie Target (by adjusting Carbs)
                                    </Button>
                                </div>
                            </TabsContent>
                             <TabsContent value="gain" className="pt-4">
                                {!muscleGainCalculation.enabled ? (<Alert variant="destructive">
                                        <Info className="h-4 w-4"/>
                                        <AlertTitle>Missing Information</AlertTitle>
                                        <AlertDescription>
                                        ...
                                        <Button type="button" onClick={() => handleApplyCalorieTarget(muscleGainCalculation.calorieTarget)} className="w-full">
                                            Apply This Calorie Target (by adjusting Carbs)
                                        </Button>
                                    </div>)}
                            </TabsContent>
