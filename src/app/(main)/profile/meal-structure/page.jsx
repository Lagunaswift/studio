...
export default function MealStructurePage() {
    ...
    const [editableMealStructure, setEditableMealStructure] = useState([]);
    ...
    const handleDeleteMealSlot = (id) => {
        setEditableMealStructure(prev => prev.filter(slot => slot.id !== id));
        setIsDirty(true);
    };
    const handleNameChange = (id, newName) => {
        setEditableMealStructure(prev => prev.map(slot => slot.id === id ? { ...slot, name: newName } : slot));
        setIsDirty(true);
    };
    const handleTypeChange = (id, newType) => {
        setEditableMealStructure(prev => prev.map(slot => slot.id === id ? { ...slot, type: newType } : slot));
        setIsDirty(true);
    };
    ...
    if (!userProfile) {
        return (<PageWrapper title="Meal Structure">
        <p>Loading user profile...</p>
      </PageWrapper>);
    }
    return (<PageWrapper title="Customize Your Meal Structure">
      <Card className="max-w-2xl mx-auto">
        ...
        <CardContent className="space-y-6">
          {editableMealStructure.map((slot, index) => (<Card key={slot.id} className="p-4 shadow-sm">
              <div className="flex flex-col sm:flex-row gap-4 items-start">
                <div className="flex-grow space-y-3">
                  <div>
                    ...
                    <Input id={`slot-name-${slot.id}`} value={slot.name} onChange={(e) => handleNameChange(slot.id, e.target.value)} placeholder="e.g., Breakfast, Lunch, Snack 1"/>
                  </div>
                  <div>
                    <label htmlFor={`slot-type-${slot.id}`} className="block text-sm font-medium text-foreground/80 mb-1">
                      Base Meal Type
                    </label>
                    <Select value={slot.type} onValueChange={(value) => handleTypeChange(slot.id, value)}>
                ...
                <Button variant="destructive" size="icon" onClick={() => handleDeleteMealSlot(slot.id)} className="mt-2 sm:mt-0 sm:ml-auto self-start sm:self-center shrink-0" aria-label="Delete meal slot">
        ...
        <CardFooter>
          <Button onClick={handleSaveChanges} disabled={!isDirty || editableMealStructure.length === 0} className="bg-accent hover:bg-accent/90 text-accent-foreground">
