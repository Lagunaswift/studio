...
import { Loader2, Sparkles, CheckSquare, Info, AlertTriangle } from 'lucide-react';
...
export default function WeeklyCheckinPage() {
    ...
    const handleApplyTargets = async () => {
        if (!recommendation?.newMacroTargets)
            return;
    ...
    if (!isSubscribed) {
        return (<PageWrapper title="Preppy: Weekly Check-in">
    ...
    return (<PageWrapper title="Preppy: Weekly Check-in">
      <div className="space-y-8">
        <Card className="shadow-lg">
          ...
          <CardContent>
             {isCheckinDisabled ? (<Alert variant="destructive">
                ...
                </Alert>) : (<Button onClick={handleRunCheckin} disabled={isLoading} className="w-full bg-accent hover:bg-accent/90 text-accent-foreground">
        ...
        {isLoading && (<div className="flex flex-col items-center justify-center h-60 text-muted-foreground">
            <Loader2 className="h-16 w-16 animate-spin text-accent mb-6"/>
            <p className="text-lg">Analyzing your progress...</p>
            <p className="text-sm">This might take a moment.</p>
          </div>)}

        {error && !recommendation && (<Alert variant={error.includes("applied") ? "default" : "destructive"} className={error.includes("applied") ? "border-green-500" : ""}>
            <Info className="h-4 w-4"/>
            <AlertTitle>{error.includes("applied") ? "Success" : "Check-in Error"}</AlertTitle>
            <AlertDescription>{error}</AlertDescription>
          </Alert>)}

        {recommendation && !isLoading && (<Card className="shadow-xl">
            <CardHeader>
            ...
            <CardContent className="space-y-6">
                ...
                <div>
                    ...
                    {recommendation.newMacroTargets ? (<MacroDisplay macros={recommendation.newMacroTargets} title="" highlightTotal className="shadow-md"/>) : <p>No new targets were recommended.</p>}
