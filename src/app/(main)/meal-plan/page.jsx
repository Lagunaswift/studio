...
export default function MealPlanPage() {
    const { userProfile, getPlannedMacrosForDate, removeMealFromPlan, updatePlannedMealServings, clearMealPlanForDate, getMealsForDate, addMealToPlan, allRecipesCache, isRecipeCacheLoading: isAppRecipeCacheLoading, updateMealStatus, } = useAppContext();
    const [selectedDate, setSelectedDate] = useState(new Date());
    const [editingMeal, setEditingMeal] = useState(null);
    const [newServings, setNewServings] = useState(1);
    const { toast } = useToast();
    const [recipePickerIndices, setRecipePickerIndices] = useState((userProfile?.mealStructure || MEAL_SLOT_CONFIG).reduce((acc, slot, index) => {
    ...
    const handleRecipePickerNavigate = (slotKey, direction) => {
        if (availableRecipesForPicker.length === 0)
            return;
    ...
    const handleAddRecipeFromPicker = async (slotKey, mealType) => {
        if (availableRecipesForPicker.length === 0)
            return;
    ...
    return (<PageWrapper title="Daily Meal Planner">
      ...
      <div className="flex flex-col lg:flex-row gap-8 mb-8">
        <div className="w-full lg:w-[380px] lg:flex-shrink-0">
          <Card className="shadow-lg">
            ...
            <CardContent>
              <Calendar mode="single" selected={selectedDate} onSelect={handleDateChange} disabled={undefined} className="rounded-md border" initialFocus={false}/>
      ...
      <section className="space-y-10">
        ...
        {isAppRecipeCacheLoading && (<div className="flex justify-center items-center min-h-[200px]">
            <Loader2 className="h-12 w-12 animate-spin text-primary"/>
            <p className="ml-4 text-muted-foreground">Loading recipes...</p>
          </div>)}

        {!isAppRecipeCacheLoading && allRecipesCache.length === 0 && (<Alert variant="destructive">
         ...
         {!isAppRecipeCacheLoading && allRecipesCache.length > 0 && availableRecipesForPicker.length === 0 && (<Alert variant="default" className="border-info">
                <Info className="h-4 w-4"/>
        ...
        {!isAppRecipeCacheLoading && availableRecipesForPicker.length > 0 && mealStructureToUse.map((slotConfig, index) => {
            ...
            return (<div key={slotKey} className="p-4 border rounded-lg shadow-md bg-card">
              <h3 className="text-xl font-semibold font-headline text-primary/90 mb-4">{slotConfig.name}</h3>
              {mealToDisplay ? (<div className="w-full max-w-lg mx-auto">
                   <Card className={cn("overflow-hidden shadow-md flex flex-col sm:flex-row transition-all", mealToDisplay.status === 'eaten' && "border-green-500 bg-green-500/5")}>
                    {mealToDisplay.recipeDetails?.image && (<div className="sm:w-1/3 relative h-32 sm:h-auto">
                        <Image src={mealToDisplay.recipeDetails.image} alt={mealToDisplay.recipeDetails.name} fill className="object-cover" data-ai-hint={mealToDisplay.recipeDetails.tags ? mealToDisplay.recipeDetails.tags.slice(0, 2).join(' ') : "food meal"}/>
                      </div>)}
                    <div className="sm:w-2/3 p-4 flex flex-col justify-between">
                      <div>
                        ...
                        {mealToDisplay.recipeDetails && (<div className="mt-2">
                            <MacroDisplay macros={{
                      ...
                      <CardFooter className="p-0 pt-4 flex gap-2 justify-between items-center">
                        <div className="flex items-center space-x-2">
                            <Checkbox id={`eaten-${mealToDisplay.id}`} checked={mealToDisplay.status === 'eaten'} onCheckedChange={(checked) => {
                  ...
                  <div className="flex items-center justify-between gap-x-2 sm:gap-x-4">
                    ...
                    <div className="flex-grow w-full max-w-md mx-auto">
                      {availableRecipesForPicker.length > 0 && (<RecipeCard recipe={availableRecipesForPicker[recipePickerIndices[slotKey] || 0]} showAddToMealPlanButton={false} showViewDetailsButton={true} className="w-full shadow-none border-0"/>)}
                    </div>
                  ...
                  {availableRecipesForPicker.length > 0 && (<Button onClick={() => handleAddRecipeFromPicker(slotKey, slotConfig.type)} className="w-full sm:w-auto mx-auto flex items-center justify-center bg-accent hover:bg-accent/90 text-accent-foreground whitespace-normal h-auto py-2" disabled={!availableRecipesForPicker[recipePickerIndices[slotKey] || 0]}>
      ...
      <section>
        ...
        {dailyMeals.length === 0 ? (<Card className="shadow-md">
            ...
            {dailyMeals.map((meal) => {
                ...
                return (<Card key={`summary-${meal.id}`} className="shadow-sm p-3">
      ...
      {editingMeal && (<Dialog open={!!editingMeal} onOpenChange={() => setEditingMeal(null)}>
          <DialogContent>
            <DialogHeader>
            ...
            <div className="py-4">
              ...
              <Input id="servings-edit" type="number" value={newServings} onChange={(e) => setNewServings(parseInt(e.target.value, 10))} min="1"/>
            </div>
            <EditDialogFooter>
