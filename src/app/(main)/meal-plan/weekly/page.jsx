...
export default function WeeklyMealPlanPage() {
    const { getMealsForDate, getPlannedMacrosForDate, allRecipesCache, isRecipeCacheLoading, userProfile, } = useAppContext();
    ...
    if (isRecipeCacheLoading && weeklyMealData.length === 0) {
        return (<PageWrapper title="Weekly Meal Plan" maxWidth="max-w-7xl">
    ...
    return (<PageWrapper title={`Weekly Meal Plan: ${weekOfLabel}`} maxWidth="max-w-7xl">
      ...
      {isRecipeCacheLoading && weeklyMealData.length > 0 && (<div className="text-center py-4 text-muted-foreground">
          <Loader2 className="inline-block h-5 w-5 animate-spin mr-2"/>
          Updating meal data...
        </div>)}

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {weeklyMealData.map(({ date, formattedDate, meals, macros }) => (<Card key={formattedDate} className="shadow-md flex flex-col h-full">
            ...
            <CardContent className="flex-grow pt-2">
              ...
              {meals.length > 0 ? (<ul className="space-y-1.5 text-xs">
                  {meals.map(meal => {
                    const recipe = meal.recipeDetails || allRecipesCache.find(r => r.id === meal.recipeId);
                    return (<li key={meal.id} className="p-1.5 bg-muted/40 rounded-md hover:bg-muted/60 transition-colors">
                        <div className="flex justify-between items-center">
                          <span className="font-medium text-primary-focus truncate" title={recipe?.name || `Recipe ID: ${meal.recipeId}`}>
                            {recipe ? (<Link href={`/recipes/${recipe.id}`} className="hover:underline">
                                {recipe.name}
                              </Link>) : (`Recipe ID: ${meal.recipeId}`)}
                          </span>
                ...
                </ul>) : (<p className="text-xs text-muted-foreground text-center py-4">No meals planned.</p>)}
            </CardContent>
          </Card>))}
      </div>
    </PageWrapper>);
