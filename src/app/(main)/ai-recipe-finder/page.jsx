...
import { suggestRecipesByIngredients } from '@/ai/flows/suggest-recipes-by-ingredients-flow';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Alert, AlertDescription, AlertTitle } from '@/components/ui/alert';
import { Loader2, Lightbulb, ChefHat, Info, CookingPot, BadgePercent, CheckCircle2, AlertTriangle, Search } from 'lucide-react';
...
export default function AIRecipeFinderPage() {
    ...
    const handleGenerateSuggestions = async () => {
        ...
        setError(null);
        setSuggestion(null);
        const recipesForAI = allRecipesCache.map(r => ({
            id: r.id,
            name: r.name,
    ...
    if (!isSubscribed) {
        return (<PageWrapper title="Preppy: Pantry Chef">
      ...
      <div className="space-y-8">
        <Card className="shadow-lg">
          ...
          <CardContent className="space-y-4">
            <Textarea placeholder="e.g., chicken breast, broccoli, onion, soy sauce" rows={4} value={ingredients} onChange={(e) => setIngredients(e.target.value)} disabled={isGenerating}/>
        ...
        {isGenerating && (<div className="flex flex-col items-center justify-center h-60 text-muted-foreground">
            <Loader2 className="h-16 w-16 animate-spin text-accent mb-6"/>
        ...
        {suggestion && !isGenerating && (<Card className="shadow-xl">
            <CardHeader>
              <CardTitle className="font-headline text-primary flex items-center">
            ...
            <CardContent className="space-y-6">
              ...
              {suggestion.suggestedRecipes.length > 0 ? (<div className="space-y-4">
                  {suggestion.suggestedRecipes.map((item) => (<Card key={item.recipeId} className="bg-card/70 border border-border hover:shadow-md transition-shadow">
                      ...
                      <CardContent className="space-y-3 text-sm">
                        ...
                        {item.missingKeyIngredients && item.missingKeyIngredients.length > 0 && (<div>
                ...
                </div>) : (<p className="text-center text-muted-foreground py-4">No specific recipes found. Try adding more ingredients or check your spelling.</p>)}
            </CardContent>
          </Card>)}
      </div>
    </PageWrapper>);
