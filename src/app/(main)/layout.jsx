...
import { UtensilsCrossed, Sparkles, ShoppingBag, CalendarDays, LayoutDashboard, PanelLeft, Target, Leaf, ListChecks, UserCircle2, BookOpen, Archive, SlidersHorizontal, Search, LogOut, FileText, Shield, CheckSquare, Settings, TrendingUp, ChefHat, ClipboardList, AlertTriangle, Database, WifiOff, Megaphone } from 'lucide-react';
import { Input } from '@/components/ui/input';
import { useState, useEffect, useTransition } from 'react';
import { Tooltip, TooltipProvider, TooltipContent, TooltipTrigger } from '@/components/ui/tooltip';
import { useAuth } from '@/context/AuthContext';
import { useAppContext } from '@/context/AppContext';
const dashboardNavItem = { href: '/', label: 'Dashboard', icon: LayoutDashboard, exact: true };
const planNavItems = [
...
function ServiceStatusBanner() {
    ...
    return (<div className="bg-yellow-500 text-center p-2 text-black text-sm font-semibold flex items-center justify-center">
...
function DevStatusIndicator() {
    ...
    return (<div className="fixed bottom-2 left-2 z-50">
        <div title={isOnline ? "Data is being sent to Firebase." : "Data is being saved to local browser storage."} className={cn("flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-semibold text-white shadow-lg", isOnline ? "bg-green-600" : "bg-orange-500")}>
        ...
        <TooltipTrigger asChild>
          <div className="relative">
              ...
              <form onSubmit={handleSearch}>
                <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search recipes..." className={cn("pl-8 group-data-[collapsible=icon]:w-8 group-data-[collapsible=icon]:p-0 group-data-[collapsible=icon]:bg-transparent group-data-[collapsible=icon]:border-none group-data-[collapsible=icon]:focus-visible:ring-0 group-data-[collapsible=icon]:focus-visible:ring-offset-0 group-data-[collapsible=icon]:cursor-pointer", "transition-all duration-300 ease-in-out")} onClick={(e) => {
            if (isCollapsed) {
...
function LayoutContent({ children }) {
    ...
    const getCurrentPageTitle = () => {
        ...
        if (pathname === '/ai-suggestions')
            return 'Preppy: Plan Generator';
        if (pathname === '/ai-recipe-finder')
            return 'Preppy: Pantry Chef';
        if (pathname === '/weekly-check-in')
            return 'Preppy: Weekly Check-in';
        // General titles
        if (pathname === '/')
            return 'Dashboard';
        if (pathname.startsWith('/recipes/add'))
            return 'Add New Recipe';
        if (pathname.match(/^\/recipes\/\d+$/))
    ...
    return (<div className="flex flex-1">
        ...
        <SidebarHeader className="p-4 border-b border-sidebar-border">
          <Link href="/" className="flex items-center gap-2">
            ...
            <div className="group-data-[collapsible=icon]:hidden">
              {isMobile ? (<SheetTitle className="text-xl font-bold font-headline text-sidebar-primary">
                   MealPlanner<span className="text-sidebar-accent">Pro</span>
                 </SheetTitle>) : (<h1 id="sidebar-title" className="text-xl font-bold font-headline text-sidebar-primary">
                  MealPlanner<span className="text-sidebar-accent">Pro</span>
                </h1>)}
            </div>
          </Link>
        ...
        <SidebarContent className="flex flex-col justify-between">
          <SidebarMenu>
            ...
            <Accordion type="multiple" className="w-full space-y-0 px-2 group-data-[collapsible=icon]:hidden">
              {mainSections.map(section => (<AccordionItem key={section.label} value={section.label.toLowerCase()} className="border-none">
            ...
            <div className="hidden flex-col gap-1 group-data-[collapsible=icon]:flex">
              {mainSections.map(section => (<Tooltip key={`collapsed-${section.label}`}>
                    <TooltipTrigger asChild>
                        <SidebarMenuItem>
      ...
      <TermsAcceptanceModal isOpen={showTerms} onAccept={handleAcceptTerms} isPending={isPending}/>
    </div>);
}
export default function MainLayout({ children, }) {
    return (<SidebarProvider defaultOpen={true} collapsible="icon">
       <TooltipProvider>
