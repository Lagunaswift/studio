...
import { suggestRecipeModification } from '@/ai/flows/suggest-recipe-modification-flow';
...
export default function RecipeDetailPage() {
    ...
    const [scaledMacros, setScaledMacros] = useState({ calories: 0, protein: 0, carbs: 0, fat: 0 });
    const [tweakRequest, setTweakRequest] = useState('');
    const [isTweaking, setIsTweaking] = useState(false);
    const [tweakError, setTweakError] = useState(null);
    const [tweakSuggestion, setTweakSuggestion] = useState(null);
    ...
    if (isLoading) {
        return (<PageWrapper title="Loading Recipe...">
    ...
    if (!recipe) {
        return (<PageWrapper title="Recipe Not Found">
    ...
    const renderRecipeTweaker = () => {
        if (!isSubscribed) {
            return (<ProFeature featureName="The Recipe Tweaker" description="Ask Preppy to modify this recipe to suit your needs! For example, 'make this vegetarian' or 'what can I use instead of almonds?'." hideWrapper/>);
        }
        return (<>
        <CardDescription>
        ...
        <CardContent className="space-y-4 pt-6">
            <Textarea placeholder="e.g., Make this vegetarian, replace mushrooms, suggest a low-carb side dish..." rows={3} value={tweakRequest} onChange={(e) => setTweakRequest(e.target.value)} disabled={isTweaking}/>
    ...
    return (<PageWrapper>
      ...
      <Card className="overflow-hidden shadow-xl rounded-lg">
        <div className="relative w-full h-64 md:h-96">
          <Image src={imageSrc} alt={recipe.name} fill sizes="(max-width: 768px) 100vw, 100vw" className="object-cover" priority data-ai-hint={imageLoadError ? aiHint : undefined} onError={handleImageError}/>
           <Button variant="ghost" size="icon" className="absolute top-3 right-3 bg-background/70 hover:bg-background/90 text-primary hover:text-accent p-2 rounded-full shadow-md" onClick={handleFavoriteToggle} aria-label={isFavorited ? "Remove from favorites" : "Add to favorites"}>
        ...
        <CardHeader className="p-6">
          ...
          <div className="mt-4 flex flex-wrap gap-4 items-center">
             <div className="flex flex-wrap gap-2">
                ...
                {recipe.chillTime && (<Badge variant="secondary" className="text-sm">
           ...
           {recipe.tags && recipe.tags.length > 0 && (<div className="mt-4 flex flex-wrap gap-2">
              {recipe.tags.map(tag => (<Badge key={tag} variant="outline" className="text-accent border-accent">{tag}</Badge>))}
            </div>)}
        </CardHeader>
        <CardContent className="p-6">
          <div className="mb-8 space-y-4">
            <h3 className="text-2xl font-semibold font-headline text-primary mb-4 flex items-center">
             ...
             <div className="flex items-center gap-4 p-4 bg-muted/50 rounded-lg">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Input id="servings-scaler" type="number" value={isNaN(displayServings) ? '' : displayServings} onChange={handleServingChange} onBlur={handleServingBlur} className="w-20 h-8 text-center" min="1"/>
            ...
            <ul className="space-y-2 list-disc list-inside bg-secondary/30 p-4 rounded-md">
                {scaledIngredients.map((ingredient, index) => (<li key={index} className="text-foreground/90">
                    {ingredient}
                    </li>))}
            </ul>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div>
              ...
              <ol className="space-y-3 list-decimal list-inside">
                {recipe.instructions.map((step, index) => (<li key={index} className="text-foreground/90 leading-relaxed">
                    {step}
                  </li>))}
              </ol>
            </div>
      ...
      {isTweaking && (<div className="flex flex-col items-center justify-center h-60 text-muted-foreground mt-4">
            <Loader2 className="h-16 w-16 animate-spin text-accent mb-6"/>
            <p className="text-lg">AI is tweaking the recipe...</p>
        </div>)}

      {tweakError && (<Alert variant="destructive" className="mt-4">
          <Info className="h-4 w-4"/>
          <AlertTitle>Error Modifying Recipe</AlertTitle>
          <AlertDescription>{tweakError}</AlertDescription>
        </Alert>)}

      {tweakSuggestion && !isTweaking && (<Card className="shadow-xl mt-4 border-accent">
            <CardHeader>
                <CardTitle className="font-headline text-primary flex items-center">
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Popover>
                ...
                <PopoverContent className="w-auto p-0">
                  <Calendar mode="single" selected={planDate} onSelect={setPlanDate} disabled={undefined} initialFocus/>
                </PopoverContent>
              </Popover>
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="mealType" className="text-right">Meal Type</Label>
              <Select value={planMealType} onValueChange={(value) => setPlanMealType(value)}>
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input id="servings" type="number" min="1" value={planServings} onChange={(e) => setPlanServings(Math.max(1, parseInt(e.target.value, 10) || 1))} className="col-span-3"/>
            </div>
          </div>
          <DialogFooter>
