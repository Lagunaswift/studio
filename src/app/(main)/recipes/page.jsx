...
function RecipesPageComponent() {
    ...
    const getIconForPreference = (preference) => {
        switch (preference.toLowerCase()) {
            case 'vegetarian':
            case 'vegan': return <Wheat className="h-4 w-4 mr-1 text-green-600"/>;
    ...
    return (<PageWrapper>
      <div className="mb-8 flex flex-col md:flex-row gap-4 items-center justify-between">
        <div className="relative flex-grow w-full md:w-auto">
          ...
          <Input type="search" placeholder="Search recipes by name, description, or tag..." className="pl-10" value={searchTerm} onChange={(e) => handleSearchChange(e.target.value)}/>
        </div>
        <div className="flex items-center space-x-4 self-start md:self-center">
          <div className="flex items-center space-x-2">
            <Switch id="favorites-filter" checked={showFavoritesOnly} onCheckedChange={setShowFavoritesOnly} aria-label="Show favorites only"/>
      ...
      {(activeDietaryFilters.length > 0 || activeAllergenFilters.length > 0) && (<Card className="mb-6 bg-secondary/30 border-accent/30">
          <CardHeader className="p-4">
            <CardTitle className="text-sm text-accent font-semibold mb-2">Filters from your profile are active:</CardTitle>
            <CardDescription className="flex flex-wrap gap-2">
              {activeDietaryFilters.map(pref => (<Badge key={pref} variant="outline" className="border-green-600 text-green-700 bg-green-100">
                  {getIconForPreference(pref)} {pref}
                </Badge>))}
              {activeAllergenFilters.map(allergen => (<Badge key={allergen} variant="outline" className="border-red-600 text-red-700 bg-red-100">
      ...
      {isRecipeCacheLoading ? (<div className="flex justify-center items-center min-h-[200px]">
          <Loader2 className="h-12 w-12 animate-spin text-primary"/>
          <p className="ml-2 text-muted-foreground">Loading recipes...</p>
        </div>) : finalRecipesForDisplay.length > 0 ? (<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {finalRecipesForDisplay.map((recipe) => {
                ...
                return (<RecipeCard key={recipe.id} recipe={recipe} onAddToMealPlan={handleOpenAddToPlanDialog} showAddToMealPlanButton={true} showViewDetailsButton={true} pantryMatchStatus={pantryMatchStatus}/>);
            })}
        </div>) : (<Card className="text-center py-10 shadow-none border-dashed">
          <CardHeader>
          ...
          <CardContent>
            ...
            {(activeDietaryFilters.length > 0 || activeAllergenFilters.length > 0 || showFavoritesOnly) && (<p className="text-sm text-muted-foreground mb-4">
      ...
      {selectedRecipe && (<Dialog open={showAddToPlanDialog} onOpenChange={setShowAddToPlanDialog}>
