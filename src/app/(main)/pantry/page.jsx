...
const UK_SUPERMARKET_CATEGORIES = [
...
export default function PantryPage() {
    ...
    const [selectedExpiryDate, setSelectedExpiryDate] = useState(undefined);
    ...
    const handleDirectQuantityUpdate = (itemId, newQuantityStr) => {
        const newQuantity = parseFloat(newQuantityStr);
        if (!isNaN(newQuantity)) {
            setEditingQuantity(newQuantity); // Keep local state for input field
        }
    };
    const saveEditedQuantity = (itemId) => {
        const item = pantryItems.find(p => p.id === itemId);
        if (item) {
    ...
    if (isAppDataLoading) {
        return (<PageWrapper title="Manage Your Pantry">
    ...
    return (<PageWrapper title="Manage Your Pantry">
      {(expiredItems.length > 0 || expiringSoonItems.length > 0) && (<div className="mb-8 space-y-4">
          {expiredItems.length > 0 && (<Alert variant="destructive">
              <AlertTriangle className="h-5 w-5"/>
              <AlertTitle>Expired Items!</AlertTitle>
              <AlertDescription>
                The following items in your pantry have expired:
                <ul className="list-disc pl-5 mt-2">
                  {expiredItems.map(item => (<li key={`expired-${item.id}`}>
                      {item.name} (Expired on: {item.expiryDate ? format(parseISO(item.expiryDate), 'dd MMMM yyyy') : 'N/A'})
                    </li>))}
                </ul>
              </AlertDescription>
            </Alert>)}
          {expiringSoonItems.length > 0 && (<Alert variant="default" className="border-orange-500 text-orange-700 dark:text-orange-400 dark:border-orange-600 [&>svg]:text-orange-500 dark:[&>svg]:text-orange-400">
              ...
              <AlertDescription>
                ...
                <ul className="list-disc pl-5 mt-2">
                  {expiringSoonItems.map(item => (<li key={`expiring-${item.id}`} className={item.expiryDate && isBefore(parseISO(item.expiryDate), new Date()) && !isSameDay(parseISO(item.expiryDate), new Date()) ? "text-red-600 dark:text-red-400 font-semibold" : ""}>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <FormField control={form.control} name="unit" render={({ field }) => (<FormItem>
                          ...
                          <Select onValueChange={field.onChange} defaultValue={field.value}>
                            ...
                            <SelectContent>
                              {COMMON_UNITS.map((unit) => (<SelectItem key={unit} value={unit}>{unit}</SelectItem>))}
                            </SelectContent>
                          </Select>
                          <FormMessage />
                        </FormItem>)}/>
                  ...
                  <FormField control={form.control} name="category" render={({ field }) => (<FormItem>
                        ...
                        <Select onValueChange={field.onChange} defaultValue={field.value}>
                          ...
                          <SelectContent>
                            {UK_SUPERMARKET_CATEGORIES.map((cat) => (<SelectItem key={cat} value={cat}>{cat}</SelectItem>))}
                          </SelectContent>
                        </Select>
                        <FormMessage />
                      </FormItem>)}/>
                  <FormField control={form.control} name="expiryDate" render={({ field }) => (<FormItem className="flex flex-col">
                        <FormLabel>Expiry Date (Optional)</FormLabel>
                        <Popover>
                          <PopoverTrigger asChild>
                            <FormControl>
                              <Button variant={"outline"} className={cn("w-full pl-3 text-left font-normal", !field.value && "text-muted-foreground")}>
                                {selectedExpiryDate ? (format(selectedExpiryDate, "dd MMMM yyyy")) : (<span>Pick a date</span>)}
                                <CalendarIcon className="ml-auto h-4 w-4 opacity-50"/>
                              </Button>
                            </FormControl>
                          </PopoverTrigger>
                          <PopoverContent className="w-auto p-0" align="start">
                            <Calendar mode="single" selected={selectedExpiryDate} onSelect={(date) => {
          ...
          <Card className="shadow-xl">
            ...
            <CardContent>
              {pantryItems.length === 0 ? (<Alert>
                  ...
                  {sortedCategories.map((category) => (<div key={category}>
                      ...
                      <ul className="space-y-2">
                        {groupedPantryItems[category].map((item) => (<li key={item.id} className="flex items-center justify-between p-3 rounded-md bg-card hover:bg-muted/50 transition-colors">
                            <div className="flex-grow">
                              ...
                              {editingItemId === item.id ? (<div className="flex items-center gap-2 mt-1">
                                  <Input type="number" value={editingQuantity} onChange={(e) => handleDirectQuantityUpdate(item.id, e.target.value)} className="h-8 w-20 text-sm" step="0.01" min="0"/>
                                ...
                                </div>) : (<div className="text-sm text-muted-foreground cursor-pointer" onClick={() => startEditing(item)}>
                                  {item.quantity.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 2 })} {item.unit}
                                </div>)}
                              {item.expiryDate && (<p className={cn("text-xs", isValid(parseISO(item.expiryDate)) && isBefore(parseISO(item.expiryDate), startOfDay(new Date())) ? "text-destructive font-semibold" : "text-muted-foreground")}>
