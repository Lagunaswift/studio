...
export default function ShoppingListPage() {
    ...
    const groupedListByAisle = useMemo(() => {
        if (activeTab !== 'aisle')
            return {};
        ...
        return mealPlan.map(pm => {
            ...
            if (recipeDetails) {
                recipeDetails.ingredients.forEach((ingStr, index) => {
                    ...
                    let correspondingShoppingListItem = shoppingList.find(sli => sli.name.toLowerCase().trim() === parsedOriginal.name.toLowerCase().trim() &&
                    ...
                    if (!correspondingShoppingListItem) {
                        correspondingShoppingListItem = shoppingList.find(sli => sli.name.toLowerCase().trim() === parsedOriginal.name.toLowerCase().trim() &&
    ...
    return (<PageWrapper title="Your Shopping List">
      <Card className="shadow-xl">
        ...
        <CardContent>
          <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value)} className="w-full">
            ...
            <TabsContent value="aisle">
              {totalCount === 0 ? (<Card className="text-center py-10 shadow-none border-dashed">
                    <CardHeader>
                 ...
                 </Card>) : categoriesByAisle.length === 0 && totalCount > 0 ? (<div className="text-center py-10 text-muted-foreground">
                    <AlertCircle className="mx-auto h-12 w-12 text-primary/50 mb-4"/>
                    <p>No items match current filters or view.</p>
                </div>) : (<div className="space-y-6">
                  {categoriesByAisle.map((category) => (<div key={category}>
                      ...
                      <div className="space-y-2">
                        {groupedListByAisle[category].map((item) => (<ShoppingListItemComponent key={item.id} item={item} onToggle={toggleShoppingListItem}/>))}
                      </div>
                    </div>))}
                </div>)}
            </TabsContent>
            <TabsContent value="recipe">
              {mealPlan.length === 0 || recipeShoppingGroups.length === 0 ? (<Card className="text-center py-10 shadow-none border-dashed">
                    <CardHeader>
                        <CardTitle className="text-xl font-semibold text-muted-foreground flex justify-center items-center">
                  ...
                  {recipeShoppingGroups.map((group) => (<div key={group.plannedMealId}>
                      <h3 className="text-lg font-semibold text-primary mb-2 capitalize border-b border-border pb-1">
                        {group.recipeName}
                      </h3>
                      <div className="space-y-2 pl-4">
                        {group.ingredients.map((ing) => (<div key={ing.id} className="flex items-center space-x-3 py-1">
                            <Checkbox id={ing.id} checked={ing.purchased} onCheckedChange={() => {
                    ...
                    }} disabled={!ing.shoppingListItemId} aria-label={`Mark ${ing.parsedName} as purchased`}/>
                            <label htmlFor={ing.id} className={`text-sm ${ing.purchased && ing.shoppingListItemId ? "line-through text-muted-foreground" : "text-foreground"}`}>
        ...
        {totalCount > 0 && (<CardFooter className="border-t pt-4">
