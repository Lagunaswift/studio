...
export default function AISuggestionsPage() {
    const { addMealToPlan, allRecipesCache, isRecipeCacheLoading: isAppRecipeCacheLoading, userProfile, isSubscribed } = useAppContext();
    const { user, isLoading: isAuthLoading } = useAuth();
    const { toast } = useToast();
    const [isGeneratingPlan, setIsGeneratingPlan] = useState(false);
    const [suggestion, setSuggestion] = useState(null);
    const [error, setError] = useState(null);
    const [recipesForAI, setRecipesForAI] = useState([]);
    ...
    const handleGeneratePlan = async () => {
        ...
        setError(null);
        setSuggestion(null);
        const mealStructureForAI = userSettingsToUse.mealStructure.map(ms => ({
            id: ms.id,
            name: ms.name,
            type: ms.type,
        }));
    ...
    const handleAddPlanToCalendar = (date) => {
        if (!suggestion || !suggestion.plannedMeals || allRecipesCache.length === 0 || !userSettingsToUse)
            return;
    ...
    if (!isSubscribed) {
        return (<PageWrapper title="Preppy: Plan Generator">
    ...
    return (<PageWrapper title="Preppy: Plan Generator">
      <div className="space-y-8">
        <Card className="shadow-lg">
          ...
          <CardContent>
            {isProfileSetupMissing && userSettingsToUse ? (<Alert variant="default" className="border-accent">
                ...
                <AlertDescription>
                  ...
                  <ul className="list-disc pl-5 mt-2 space-y-1">
                    {!userSettingsToUse.mealStructure || userSettingsToUse.mealStructure.length === 0 ? (<li>Defined your <Link href="/profile/meal-structure" className="underline hover:text-accent">Meal Structure</Link>.</li>) : null}
                    {!userSettingsToUse.macroTargets ? (<li>Set your <Link href="/profile/targets" className="underline hover:text-accent">Macro Targets</Link>.</li>) : null}
                     {(!userSettingsToUse.dietaryPreferences || userSettingsToUse.dietaryPreferences.length === 0) && (!userSettingsToUse.allergens || userSettingsToUse.allergens.length === 0) ? (<li>Configured your <Link href="/profile/diet-type" className="underline hover:text-accent">Diet Type</Link> and <Link href="/profile/allergens" className="underline hover:text-accent">Allergens</Link>.</li>) : null}
                  </ul>
                   <Button onClick={handleGeneratePlan} disabled={isGeneratingPlan || isAppRecipeCacheLoading || recipesForAI.length === 0} className="w-full mt-4 bg-accent hover:bg-accent/90 text-accent-foreground">
                      {isGeneratingPlan || isAppRecipeCacheLoading ? (<Loader2 className="mr-2 h-4 w-4 animate-spin"/>) : (<Send className="mr-2 h-4 w-4"/>)}
                      Generate Plan Anyway
                  </Button>
                </AlertDescription>
              </Alert>) : (<Button onClick={handleGeneratePlan} disabled={isGeneratingPlan || isAppRecipeCacheLoading || recipesForAI.length === 0} className="w-full bg-accent hover:bg-accent/90 text-accent-foreground">
                {isGeneratingPlan || isAppRecipeCacheLoading ? (<Loader2 className="mr-2 h-4 w-4 animate-spin"/>) : (<Send className="mr-2 h-4 w-4"/>)}
                Generate My Meal Plan
              </Button>)}
            {isAppRecipeCacheLoading && <p className="text-sm text-muted-foreground mt-2 text-center">Loading recipe data for AI...</p>}
            {!isAppRecipeCacheLoading && recipesForAI.length === 0 && !isGeneratingPlan && (<p className="text-sm text-muted-foreground mt-2 text-center">No recipes found in the database. Add recipes to enable AI planning.</p>)}
          </CardContent>
        </Card>

        {isGeneratingPlan && (<div className="flex flex-col items-center justify-center h-60 text-muted-foreground">
            <Loader2 className="h-16 w-16 animate-spin text-accent mb-6"/>
        ...
        {suggestion && !isGeneratingPlan && (<Card className="shadow-xl">
            <CardHeader>
              <CardTitle className="font-headline text-primary flex items-center">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                return (<Card key={item.mealSlotId} className="bg-card/70 border border-border hover:shadow-md transition-shadow">
                      ...
                      <CardContent>
                        {recipeDetails && item.calculatedMacros ? (<MacroDisplay macros={item.calculatedMacros} title="Macros for this meal" className="shadow-none border-none text-sm"/>) : (<p className="text-sm text-muted-foreground">Macro details unavailable.</p>)}
                      </CardContent>
                    </Card>);
            })}
              </div>
              ...
              <div>
               ...
               {userSettingsToUse?.macroTargets && (<div className="mt-4">
                    ...
                    return (<div key={key} className="p-2 bg-muted/30 rounded">
