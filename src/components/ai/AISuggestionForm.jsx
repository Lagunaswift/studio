...
export function AISuggestionForm({ onSubmit, isLoading }) {
    const form = useForm({
      ...
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
            <FormField control={form.control} name="dietaryGoals" render={({ field }) => (<FormItem>
            ...
            <FormField control={form.control} name="preferences" render={({ field }) => (<FormItem>
                  <FormLabel>Preferences & Allergies</FormLabel>
                  <FormControl>
                    <Textarea placeholder="e.g., Vegetarian, no nuts, love spicy food, dislike cilantro" {...field} rows={3}/>
                  </FormControl>
                  <FormMessage />
                </FormItem>)}/>
