...
export function TermsAcceptanceModal({ isOpen, onAccept, isPending }) {
    const [isChecked, setIsChecked] = useState(false);
    return (<Dialog open={isOpen}>
      <DialogContent className="sm:max-w-md" hideCloseButton onInteractOutside={(e) => {
            e.preventDefault();
        }} onEscapeKeyDown={(e) => {
            e.preventDefault();
        }}>
        ...
        <div className="flex items-center space-x-2 mb-4">
          <Checkbox id="terms-acceptance" checked={isChecked} onCheckedChange={(checked) => setIsChecked(checked)}/>
        ...
        <DialogFooter>
          <Button onClick={onAccept} disabled={!isChecked || isPending} className="w-full bg-accent hover:bg-accent/90 text-accent-foreground">
